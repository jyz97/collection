...
//        recoveryManager = new DummyRecoveryManager();
    ...
    private class TransactionContextImpl extends AbstractTransactionContext {
        ...
        public Iterator<Record> sortedScan(String tableName, String columnName) {
            ...
            LockContext lcTable = getTableContext(tableName);
            LockUtil.ensureSufficientLockHeld(lcTable, LockType.S);
        ...
        public void close() {
            // TODO(hw4_part2): release locks held by the transaction
               // the list of locks locks held by this TRANSACTION, in order of acquisition.
            List<Lock> locks = lockManager.getLocks(this);
            Collections.reverse(locks);
            LockContext lc;
            for (Lock lock: locks) {
                lc = LockContext.fromResourceName(lockManager, lock.name);
                lc.release(this);
            }
        }
