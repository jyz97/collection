...
class BNLJOperator extends JoinOperator {
    ...
    private class BNLJIterator extends JoinIterator {
        ...
        private Record rightRecord = null;  // equals null when first assigned and reach the end of one page
        private int maxPage = numBuffers - 2;
        ...
        private void fetchNextLeftBlock() {
            ...
            this.leftRecordIterator = BNLJOperator.this.getBlockIterator(this.getLeftTableName(), this.leftIterator, this.maxPage);
        ...
        private void fetchNextRightPage() {
            ...
            if (this.rightRecordIterator.hasNext()) {   // make sure fetched page is not empty
...
//            if (this.leftRecord == null) { throw new NoSuchElementException("No new record to fetch"); }
            ...
            do {
                if (this.leftRecord == null) { throw new NoSuchElementException("No new record to fetch"); }
                this.nextRecord = null;
                if (this.rightRecord != null) { // regular
                ...
                } else { // finish loop through one left record with one right page, move to next left record
                    if (this.leftRecordIterator.hasNext()) { // update left record within the same block
                                                             // update right record to be the first one that page
                        this.leftRecord = this.leftRecordIterator.next();
                        this.resetRightRecordIterator();
                    } else if (this.rightIterator.hasNext()){ // update left record to be the first on that block
                                                              // update right record to be on the next right page
                        this.resetLeftRecordIterator();
                        this.fetchNextRightPage();
                    } else { // update left record to be on the next block
                             // update right record to be the very first one
                        this.fetchNextLeftBlock();
                        this.resetRightIterator();
                    }
                }
            } while (!hasNext()); // finish loop through all left blocks
...
//                            this.rightRecord = this.rightRecordIterator.next();
//                        } else { // finish loop through one left record with one right page
//                            this.leftRecord = this.leftRecordIterator.next();
//                            this.resetRightRecordIterator();
//                        }
//                    } else { // finish loop through one left block with one right page
//                        this.fetchNextRightPage();
//                        this.resetLeftRecordIterator();
//                    }
//                } else { // finish loop through one left block with all right pages
//                    this.fetchNextLeftBlock();
//                    this.resetRightIterator();
//                }
