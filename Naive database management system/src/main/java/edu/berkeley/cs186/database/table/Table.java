...
public class Table implements BacktrackingIterable<Record> {
    ...
    // boolean flag of autoEscalate or not, my modification
    ...
    public Table(String name, Schema schema, HeapFile heapFile, LockContext lockContext) {
        ...
        // ensure X lock for full table scan or construct a new table, my modification
    ...
    public synchronized Record updateRecord(List<DataBox> values, RecordId rid) {
        ...
        // acquire X explicitly, my modification
        LockContext lcPage = lockContext.childContext(rid.getPageNum());
        LockUtil.ensureSufficientLockHeld(lcPage, LockType.X);
    ...
    public BacktrackingIterator<RecordId> ridIterator() {
        ...
        // ensure S lock for full table scan, my modification
