...
public class Histogram {
    ...
    public void buildHistogram(Table table, int attribute) {
        ...
        int bucketNumber = this.buckets.length;

        //1. first calculate the min and the max values
        BacktrackingIterator<Record> recordIter = table.iterator();
        Record r;
        float cur;
        ...
        while (recordIter.hasNext()) {
            r = recordIter.next();
            cur = this.quantization(r, attribute);
            if (cur > max) {max = cur;}
            if (cur < min) {min = cur;}
        }
        ...
        this.minValue = min;
        float diff = max - min;
        this.width = diff / bucketNumber;

        //3. create each bucket object
        for (int i = 0; i < bucketNumber; i++) {
            this.buckets[i] = new Bucket<>(minValue + i * width, minValue + (i + 1) * width);
        }

        //4. populate the data using the increment(value) method
        int index;
        recordIter = table.iterator();

        while (recordIter.hasNext()) {
            r = recordIter.next();
            cur = this.quantization(r, attribute);
            index = bucketIndex(cur);
            this.buckets[index].increment(cur);
        }
    }
    ...
    private float [] allEquality(float qvalue) {
        ...
        if ((qvalue >= this.minValue) && (qvalue <= this.maxValue)) {
            int index = this.bucketIndex(qvalue);
            int distinctCount = this.buckets[index].getDistinctCount();
            result[index] = 1.0f / distinctCount;
        }
    ...
    private float [] allNotEquality(float qvalue) {
        ...
        if ((qvalue >= this.minValue) && (qvalue <= this.maxValue)) {
            int index = this.bucketIndex(qvalue);
            int distinctCount = this.buckets[index].getDistinctCount();
            result[index] = 1 - 1.0f / distinctCount;
        }
    ...
    private float [] allGreaterThan(float qvalue) {
        ...
        float end = this.buckets[index].getEnd();
        for (int k = 0; k < index; k++) {
            result[k] = 0.0f;
        }
        result[index] = (end - qvalue) / this.width;
        for (int k = index + 1; k < bucketNumber; k++) {
            result[k] = 1.0f;
        }
    ...
    private float [] allLessThan(float qvalue) {
        ...
        float start = this.buckets[index].getStart();
        for (int k = 0; k < index; k++) {
            result[k] = 1.0f;
        }
        result[index] = (qvalue - start) / this.width;
        for (int k = index + 1; k < bucketNumber; k++) {
            result[k] = 0.0f;
        }
