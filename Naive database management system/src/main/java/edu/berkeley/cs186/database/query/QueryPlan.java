...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        // pass 1
        HashMap<Set, QueryOperator> pass1Map = new HashMap<>();
        Set<String> tableName;
        QueryOperator minOp;
        List<String> allTables = new ArrayList<>(this.joinTableNames);
        allTables.add(0, this.startTableName);

        for (String tn: allTables) {
            tableName = new HashSet<>();
            tableName.add(tn);
            minOp = this.minCostSingleAccess(tn);
            pass1Map.put(tableName, minOp);
        }
        HashMap<Set, QueryOperator> prevMap = new HashMap<>(pass1Map);
        // pass i
        for (int i = 1; i < allTables.size(); i++) {
            prevMap = (HashMap<Set, QueryOperator>)this.minCostJoins(prevMap, pass1Map);
        }
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        minOp = sequentialOp;
        int minIOCount = minOp.estimateIOCost();
        ...
        String indexColumn;
        for (int i: indices) {
            indexColumn = this.selectColumnNames.get(i);
            operator = this.selectOperators.get(i);
            value = this.selectDataBoxes.get(i);
            op = new IndexScanOperator(this.transaction, table, indexColumn, operator, value);
            ioCount = op.estimateIOCost();
            if (ioCount < minIOCount) {
                minIOCount = ioCount;
                minOp = op;
                indexIOMin = i;
            }
        ...
        minOp = this.addEligibleSelections(minOp, indexIOMin);
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        for (Set s: prevMap.keySet()) {
            ...
            for (int i = 0; i < this.joinTableNames.size(); i++) {
                leftTableColumn = this.getJoinLeftColumnNameByIndex(i);
                rightTableColumn = this.getJoinRightColumnNameByIndex(i);
                newTableNames = new HashSet<>();
                if (s.contains(leftTableColumn[0]) && (!s.contains(rightTableColumn[0]))) {
                    newTableNames.add(rightTableColumn[0]);
                    leftOp = op;
                    rightOp = pass1Map.get(newTableNames);
                    newTableNames.addAll(s);
                    leftColumn = leftTableColumn[1];
                    rightColumn = rightTableColumn[1];
                } else if ((!s.contains(leftTableColumn[0])) && (s.contains(rightTableColumn[0]))) {
                    newTableNames.add(leftTableColumn[0]);
                    leftOp = op;
                    rightOp = pass1Map.get(newTableNames);
                    newTableNames.addAll(s);
                    leftColumn = rightTableColumn[1];
                    rightColumn = leftTableColumn[1];
                } else {
                    continue;
                }
                ...
                if ((!map.containsKey(newTableNames)) || (minOp.estimateIOCost() < map.get(newTableNames).estimateIOCost())) {
                    map.put(newTableNames, minOp);
                }
            }
        }
