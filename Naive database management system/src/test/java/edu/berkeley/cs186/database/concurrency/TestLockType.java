...
public class TestLockType {
    ...
    @Test
    public void testCanBeParentGeneral() {
        for (LockType lockType : LockType.values()) {
            if ((lockType == LockType.IS) || (lockType == LockType.IX)) {
                assertTrue(LockType.canBeParentLock(lockType, LockType.S));
                assertTrue(LockType.canBeParentLock(lockType, LockType.IS));
            }
            else {
                assertFalse(LockType.canBeParentLock(lockType, LockType.S));
                assertFalse(LockType.canBeParentLock(lockType, LockType.IS));
            }
        }

        for (LockType lockType : LockType.values()) {
            if ((lockType == LockType.IX) || (lockType == LockType.SIX)) {
                assertTrue(LockType.canBeParentLock(lockType, LockType.X));
                assertTrue(LockType.canBeParentLock(lockType, LockType.IX));
            }
            else {
                assertFalse(LockType.canBeParentLock(lockType, LockType.X));
                assertFalse(LockType.canBeParentLock(lockType, LockType.IX));
                assertFalse(LockType.canBeParentLock(lockType, LockType.SIX));
            }
        }
    }
