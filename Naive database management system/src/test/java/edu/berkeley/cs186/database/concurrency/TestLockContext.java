...
public class TestLockContext {
    ...
    @Test
    public void testPromoteSaturation() {
        LockContext tableContext = dbLockContext.childContext("table2", 2);
        LockContext pageContext = tableContext.childContext("page1", 1);
        TransactionContext t1 = transactions[1];
        dbLockContext.capacity(10);
        dbLockContext.acquire(t1, LockType.IX);
        tableContext.capacity(10);
        tableContext.acquire(t1, LockType.IS);
        pageContext.acquire(t1, LockType.S);
        assertEquals(0.1, dbLockContext.saturation(t1), 1E-6);
        assertEquals(0.1, tableContext.saturation(t1), 1E-6);
        dbLockContext.promote(t1, LockType.SIX);
        assertEquals(0.0, dbLockContext.saturation(t1), 1E-6);
        assertEquals(0.0, tableContext.saturation(t1), 1E-6);
    }
